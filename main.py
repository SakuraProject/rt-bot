...
bot.load_extension("cogs.system._first")
# スラッシュマネージャーを設定する
...
async def on_ready():
    ...
    bot.unload_extension("cogs.system._first")

    # 拡張を読み込む
    setup(bot)  # rtlib.setup
    bot.load_extension("cogs.role._oldrole")  # oldroleだけ特別に読み込んでいる
    for cat in listdir("cogs"):
        if not cat.startswith(("_", ".")):
            for name in listdir("cogs/" + cat):
                if not name.startswith(("_", ".")):
                    try:
                        bot.load_extension(
                            f"cogs.{cat}.{name[:-3] if name.endswith('.py') else name}"
                        )
